    ...
    >
        ...
        <div class="ml-2.5 flex items-center gap-2">
            <div
                ...
                :key="option"
                class="flex items-center gap-1 bg-gray-800 py-1 pl-2 pr-1 text-xs"
            >
                <span>{{ option }}</span>
            </div>
            ...
            <input
                ...
                class="peer block h-10 w-full border-none bg-transparent placeholder-transparent focus:border-transparent focus:outline-none focus:ring-0"
            />

            <span
                v-if="selectedOptions.length >= 2"
                @click="$emit('clearSelections')"
                class="absolute right-0 bg-gray-800 p-3 text-sm text-gray-400 transition duration-150 ease-in-out hover:cursor-pointer hover:bg-gray-700"
